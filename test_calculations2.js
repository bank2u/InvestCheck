// Test the updated projection calculations
function calculateProjections(monthlyAmount, years, annualInvestmentReturn) {
  ...
  for (let year = 0; year <= years; year++) {
    ...
    if (year < years) {
      ...
      investmentBalance = (investmentBalance + yearlyDeposit) * (1 + annualInvestmentReturn);
      bankBalance = (bankBalance + yearlyDeposit) * (1 + bankReturnRate);
    }
  }
...
// Show year 5 as well for completeness
console.log("\nYear 5:");
console.log(`  Investment: ฿${projections[5].investmentBalance}`);
console.log(`  Bank: ฿${projections[5].bankBalance}`);
